...
| 成员名册 | **会员展示**（以用户为行，带主力角色与绑定标识）+ **公会角色展示**（以角色为行，带隶属成员与绑定标识）+ 引荐结构树形图 |
| 数据分析 | 职业分布、装等分布、团本进度可视化 |
| 工具分享 | 插件库（含 WeakAuras 字符串一键复制）+ 成员发布的攻略分享（可传附件） |
| 活动日历 | 团本、大秘境、PVP 活动的报名、替补队列、出勤标记、名单导出（CSV）与 `.ics` 日历订阅 |
| 媒体画廊 | 击杀截图、活动合照、视频集锦 |
| 账号系统 | 邮箱注册 + 邮箱验证、密码登录、找回密码（邮件一次性链接）、**个人中心可改昵称与密码**、入会审批 |
| 角色管理 | 从 WTF 目录**导入游戏角色**（可选是否把文件一并备份到服务器）；自动识别 账号/服务器/角色，最多 5 个账号 / 20 个角色，可排序、**可指定唯一主力**、可打包导出 zip |
...
> 因此审批通过时**也不再自动创建角色**。
>
> **导入只认公会名单**：WTF 里的角色会与 Raider.IO 公会名单比对（服务器名
> 中英自动对齐），**非公会角色不导入**——不展示、不上传、不建档。
> 导入时可选是否把 WTF 文件**备份到服务器**（见下）。
>
...
- `Application` —— 入会申请（申请编号、审核状态）。**游戏信息字段已改为可空**：
  ...
  > ⚠️ **`GET /api/applications` 只返回调用者本人的申请，且不含已通过的。**
  > 后台审批页的数据由服务端组件直接查库，不经过这个接口 —— 所以这个
  > 接口**只服务于「个人中心」**。它曾经写成「返回全站所有申请 + `include user`」，
  > 于是**任何已通过成员都能看到别人的申请，网络面板里连邮箱都能看到**；
  > `isMemberOrAboveRole` 挡住的是未登录/待审批者，挡不住「成员 A 看成员 B」。
  > 权限判断看的是 **`role`**（注册给 `USER`，通过后才变 `MEMBER`），
  > 不是 `status` —— 见 `lib/page-guard.ts`。

### 名册的两个维度

站点的**主维度是用户**，角色是「被用户绑定的东西」——用户与角色之间**只有一层绑定关系**。
所以 `/roster` 分成两个视图，两个视图都要能回答「绑没绑」：

| 视图 | 行 | 带什么 |
| --- | --- | --- |
| **会员展示** | 用户 | 昵称、会阶、**主力角色**、✓ 绑定标识 |
| **公会角色展示** | 公会角色 | 名称、服务器、职业/专精/职能、**隶属成员**、✓ 绑定标识 |

映射逻辑是纯函数，在 **`lib/roster-view.ts`**（`toDisplay` / `buildMemberRows` /
`buildCharacterRows`）——抽出来是为了能单测，这里最容易写错。

两条规则值得记住：

1. **职业/专精/职能以公会名单为准**，手工字段只作兜底。
   这些值在 WTF 导入时就从 Raider.IO 一并拿到了，绑定后直接可用，
   不需要管理员再手填一遍。手工兜底是为了「非公会角色」那条历史路径。
2. **服务器名以当前字典为准**。字典（`GameRealm`）管理员可随时重刷，
   而 `Character.server` 存的是**导入当时**的名字 —— 直接拿来显示会用到过期数据。
   所以展示时按 `realmSlug` 回查字典。
- `Character` —— 魔兽角色。`faction` / `class` / `spec` / `role` **可空** ——
  ...
  > ⚠️ **`@@unique([server, name])` —— 同一服务器的角色名全站唯一。**
  > 一个魔兽角色在游戏里只属于一个人，不允许两个人各自认领同一个：
  > 既会重复统计，也给冒充留口子。因此：
  > - 导入时若角色已被他人认领 → **409**，并点名是谁占的（不是默默跳过）
  > - `server` 写入前统一经 `canonicalRealmName()` 归一成字典里的**中文规范名** ——
  >   否则「回音山」与 `Echo Ridge` 并存会**绕过唯一约束**
  > - 应用层先查再插只是为了让报错能读懂，**真正的保证在数据库唯一索引**上；
  >   并发竞态由 `P2002` 兜底

- `WtfAccount` —— 成员导入过的 WTF 账号（**每位最多 5 个**）。
  数据库只记录账号名与服务器数。路径里 `Account/<账号>/SavedVariables/`
  属**账号级**插件数据，不会被误当成服务器。
- `GuildMember` —— **公会名单**（Raider.IO 导入，见上）。`Character.guildMemberId`
  指过来就代表「是公会成员」——这是唯一判据，不另设布尔标志位（会漂移）。
  后台名单因此把成员分成两类：**已验证**（有站内角色绑定，显示用户名）
  与**未绑定用户**（名单里有、但站内还没有人认领）。
- `Character.isMain` 为**主力**，每位成员**至多一个**（设置时在同一事务里先清空旧的）。

> ⚠️ **`Character.userId` 可空且 `ON DELETE SET NULL`** —— 删账号**不会**删角色。
> 曾经因此出过问题：被驳回的申请人若先导入过 WTF，账号一删角色就变成
> 「孤儿」（`userId = null`），而 `isPublic` 仍是 `true`，会**继续挂在成员名册上公开可见**。
> 现在两手都堵：驳回事务里**显式先删角色**，`/roster` 查询也加 `userId: { not: null }` 兜底。
...
- **会话里的用户信息必须每次从库里刷新**（`lib/auth.ts` 的 `jwtCallback`）。
  ...
  而且非重新登录不可。目前 `role` / `status` / **`name`** 都已每次刷新。
  改动用户信息后，前端要 `await update()` 刷新会话，光 `router.refresh()` 不够。
- **昵称规则只有一份**：`lib/validations.ts` 的 `nicknameSchema`，
  注册与「个人中心改昵称」共用，别在两处各写一遍。
